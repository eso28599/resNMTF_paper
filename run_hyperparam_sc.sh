#!/bin/bash
#PBS -l select=1:ncpus=8:mem=100gb
#PBS -l walltime=01:00:00
#PBS -N single_cell
#PBS -o /rds/general/user/eso18/home/resNMTF_paper/RealData/single_cell/logs/run_ssvd.out
#PBS -e /rds/general/user/eso18/home/resNMTF_paper/RealData/single_cell/logs/run_ssvd.err


module load  R/4.4.2-gfbf-2024a 
export R_LIBS_USER=/rds/general/user/eso18/home/R/x86_64-pc-linux-gnu-library/4.3  
cd resNMTF_paper
# Rscript --vanilla -e 'install.packages("philentropy", type = "source", repos="https://cloud.r-project.org")'

# Rscript --vanilla RealData/3cancers/hyperparam.r
# 
# Rscript --vanilla RealData/3sources/3sources_other_results.r
Rscript --vanilla RealData/single_cell/single_cell_other_results.r

# # # python scripts
# eval "$(~/miniforge3/bin/conda shell.bash hook)"
# conda activate resnmtf_env
# # python3 RealData/3sources/3sources_python_analysis.py
# python3 RealData/single_cell/single_cell_python_analysis.py

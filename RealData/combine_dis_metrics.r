...
res_list <- list(
  results_euc[results_euc[, "psi"] != 0, 1:n_col],
...
for (i in 1:3) {
  ...
  sub_res[[i]] <- as.data.frame(cbind(
    method,
    res[c(max_bis_e, max_bis_c, max_bis_m, max_f), ]
  ))
...
if (dataset == "single_cell") {
  ...
  cor_bis <- cor(euc_sc$F.score, euc_sc$BiS.E, method = "spearman")
  cor_sil <- cor(euc_sc$F.score, euc_sc$Sil, method = "spearman")
  cor_df <- data.frame(
    "bis" = cor_bis,
    "sil" = cor_sil
  )
  write.csv(cor_df, paste0(base_path, "/cor_bis_sil.csv"), row.names = FALSE)
  sc <- as.data.frame(euc_sc)
  p <- ggplot(sc, aes(x = psi)) +
    geom_point(
      aes(
        y = F.score,
        color = factor("F.score", levels = c("F.score", "BiS", "Sil"))
      ),
      alpha = 0.55
    ) +
    ...
    scale_colour_manual(
      ...
      labels = c(
        "F.score" = " F score (left axis)",
        "BiS" = "Bisilhouette (right axis)",
        "Sil" = "Silhouette (right axis)",
  ...
  suppressMessages(ggsave(paste0(base_path, "/f_score_bis_sil.pdf"),
    plot = p, compress = FALSE, device = "pdf", width = 8, height = 8
  ))
}

...
data = [
    np.array(pd.read_excel(data_views, sheet))
    for sheet in data_views.sheet_names
]
n_views = len(data)
...
for j in method_idx:
    ...
    if n_clusts == 0:
        row_clusts = [
            pd.DataFrame([0 for i in np.arange(k)])
            for k in [n_samps for j in np.arange(n_views)]
        ]
        ...
        col_clusts = fix_col_clusts(
            iSSVD_applied["Variable_index"], n_views, n_clusts, n_vars
        )

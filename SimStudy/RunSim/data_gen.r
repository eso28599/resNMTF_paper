...
if(factor=="noise"){
   ...
   for (i in 1:length(method_vec)){
        ...
        save_data(row_cl_dims[i][[1]], col_cl_dims[i][[1]],
                 k_vals[i],
                path, noise_level,  row_same_shuffle=row_same_shuffle,col_same_shuffle=col_same_shuffle,signal=signal_vec[i])
}
  ...
  for (i in 1:length(method_vec)){
        ...
        save_data(row_cl_dims[i][[1]], col_cl_dims[i][[1]],
                 k_vals[i],
                path, noise_level,row_e[i], col_e[i], row_o[i], col_o[i], row_same_shuffle=row_same_shuffle,col_same_shuffle=col_same_shuffle)
  }
}else{
  for (i in 1:length(method_vec)){
        #generate data and save in given path
        ...
        save_data(row_cl_dims[i][[1]], col_cl_dims[i][[1]],
                 k_vals[i],
                path, noise_level, row_same_shuffle=row_same_shuffle,col_same_shuffle=col_same_shuffle)
}
}

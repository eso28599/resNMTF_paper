...
make_plot_line <- function(data_frame, cluster, plot_title, sub_title, x_title, y_title, filename, measures){
    ...
    p <- ggplot(subset(data_frame, Choice==cluster), aes(x=Factor, y=Mean,
    ...
    p <- p+
        ...
        axis.ticks = element_line(size = 0.3), axis.line = element_line(size = 0.3))
...
all_table <- function(df,measures, filename, repeats, col_names_list, subheading, table_head, ndig = 4,feat = "Method", group=FALSE){

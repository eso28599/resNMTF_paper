...
if(factor == "signal"){ 
    ...
    for (i in 1:length(method_vec_issvd)){
        ...
        true_rows <- import_matrix(paste0(data_name, "/issvd_",i, "/true_rows.xlsx"))
        true_cols <- import_matrix(paste0(data_name,  "/issvd_",i, "/true_cols.xlsx"))
        data_views <- import_matrix(paste0(data_name,  "/issvd_",i,"/data.xlsx"))

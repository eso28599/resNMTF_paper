...
averages <- c("Overall")
results <- data.frame(Method = c(), Choice = c(),
                      Measure = c(), Mean = c(), Sd = c())
# analyse each method
for (k in seq_along(file_names)) {
  ...
  for (i in 1:n_repeats) {
    ...
    avgs <- get_averages(
      avgs, data_name,
      paste0(file_names[k], "_results"), i, measures
    )
  }
...
plot_res_nmtf <- make_plot_line_issvd(
  results_nmtf, "Overall", x_title, "Measure",
  path_to_save, measures
)
if (phi_constant) {
  # now produce one plot for each measure
  measures <- c(
    "Relevance", "Recovery", "F score",
...
col_names <- c("Measure", "F score", "Relevance",
               "Recovery", "CSR", "BiS", "Restrictions")

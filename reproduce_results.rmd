...
2. The code chunks to reproduce the results on the real data (which take substantially longer to run (at least a day each) than the simulations) have been set to `eval=FALSE` and `include=FALSE` to prevent the code from running. 

To reproduce these results anyway, simply remove these two arguments.
</div>
...
```{zsh, messages=FALSE, results='hide'}
export sim="bicl"

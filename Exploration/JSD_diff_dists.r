...
save_data(
  ...
  4,
  "Exploration", 3,
  row_same_shuffle = TRUE, col_same_shuffle = FALSE
)

data4 <- import_matrix("Exploration/data.xlsx")
nmtf_results4 <- apply_resnmtf(data4, phi = phi)
...
i <- 1
for (j in 1:n_bicls) {
  dens1 <- density(nmtf_results4$output_f[[i]][, j],
    from = 0, to = max(nmtf_results4$output_f[[i]])
  )
  dens2 <- density(nmtf_shuffled$output_f[[i]][, j],
    from = 0, to = max(nmtf_results4$output_f[[i]])
  )
  m_fac1 <- 1 +
    sum(dens1$y[dens1$x > max(nmtf_results4$output_f[[i]][, j])]) /
      sum(dens1$y[dens1$x < max(nmtf_results4$output_f[[i]][, j])])
  m_fac2 <- 1 +
    sum(dens2$y[dens2$x > max(nmtf_shuffled$output_f[[i]][, j])]) /
      sum(dens2$y[dens2$x < max(nmtf_shuffled$output_f[[i]][, j])])
  dens1$y[dens1$x > max(nmtf_results4$output_f[[i]][, j])] <- 0
  dens2$y[dens2$x > max(nmtf_shuffled$output_f[[i]][, j])] <- 0

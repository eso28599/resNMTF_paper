...
nmtf_shuffled <- restMultiNMTF_run(Xinput = updated_data, 
            phi = phi, KK=rep(4,2))
...
for(i in 1:1){
    for(j in 1:n_bicls){
        ...
        dens2 <- density(nmtf_shuffled$Foutput[[i]][,j],
             from=0, to=max(nmtf_results4$Foutput[[i]]))
        m_fac1 <- 1 +
         sum(dens1$y[dens1$x>max(nmtf_results4$Foutput[[i]][,j])])/ sum(dens1$y[dens1$x<max(nmtf_results4$Foutput[[i]][,j])])
        m_fac2 <- 1 +
         sum(dens2$y[dens2$x>max(nmtf_shuffled$Foutput[[i]][,j])])/ sum(dens2$y[dens2$x<max(nmtf_shuffled$Foutput[[i]][,j])])
...
figure <- ggarrange(plotlist=plots_true[1:4], # remove axis labels from plots
                    labels = NULL,
                    ncol = 2, nrow = 2,
                    align = "hv", 
                    font.label = list(size = 10, color = "black", face = "bold", family = NULL, position = "top"))

figure <- annotate_figure(figure, left = textGrob("Density", rot = 90, vjust = 1, gp = gpar(cex = 1.3)),
                    bottom = textGrob("F entry", gp = gpar(cex = 1.3)))

figure_shuff <- ggarrange(plotlist=plots_shuff[1:4], # remove axis labels from plots
                    labels = NULL,
                    ncol = 2, nrow = 2,
                    align = "hv", 
                    font.label = list(size = 10, color = "black", face = "bold", family = NULL, position = "top"))
